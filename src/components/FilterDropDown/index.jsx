...
export default class FilterDropDown extends Component {
    ...
    this.setState(
      {
        ...
        shadowActiveMenuArr: JSON.parse(
    ...
    if (prevProps.defaultSelected !== this.state.defaultSelected) {
      this.setState(
        ...
        () => {
          ...
          this.setState(
            {
              defaultActive: JSON.parse(
                JSON.stringify(prevProps.defaultSelected)
              ),
              activeMenuArr: JSON.parse(
                JSON.stringify(prevProps.defaultSelected)
              ),
              shadowActiveMenuArr: JSON.parse(
                JSON.stringify(prevProps.defaultSelected)
  ...
  initMenu = () => {
    ...
    for (let i = 0; i < this.state.filterData.length; i++) {
      ...
      tmpMenu.push({
        ...
        name:
          tmpitem.name ||
          (tmpitem.type == "filter" ? "筛选" : tmpitem.submenu?.[0].name),
        type: tmpitem.type,
      });
      //初始化选中项数组-ui状态
    ...
    this.setState(
      ...
      () => {
        //初始化选中项数组
        tmpMenuActiveArr =
          this.state.defaultActive.length > 0
            ? this.state.defaultActive
            : this.state.activeMenuArr.length > 0
            ? this.state.activeMenuArr
            : tmpMenuActiveArr;
        this.setState(
          {
            defaultActive: [],
  ...
  showMoreSub = (index) => {
    ...
    subData[this.state.showPage].submenu[
      this.state.activeMenuArr[this.state.showPage][0]
    ].submenu[index].showAllSub = true;
    this.setState(
      {
        subData,
      },
  ...
  selectHierarchyMenu = (
    ...
    if (
      ...
      level2_index == null &&
      level3_index == null &&
      this.state.shadowActiveMenuArr[page_index][0] == level1_index
    ) {
      activeMenuArr.splice(
        page_index,
        1,
        JSON.parse(JSON.stringify(this.state.shadowActiveMenuArr[page_index]))
      );
      this.setState({
        activeMenuArr,
      });
    ...
    this.setState(
      ...
      () => {
        ...
        if (
          ...
          level2_index != null ||
          (level1_index != null &&
            this.state.subData[page_index].submenu[level1_index].submenu
              .length == 0)
        ) {
          let sub =
            this.state.subData[page_index].submenu[level1_index].submenu[
          ...
          if (this.props.updateMenuName) {
            ...
            menu[page_index].name =
              (level3_index != null && sub.submenu[level3_index].name) ||
              (level2_index != null && sub.name) ||
              this.state.subData[page_index].submenu[level1_index].name;
            this.setState({
              menu,
            });
          }
          ...
          shadowActiveMenuArr[page_index] =
            this.state.activeMenuArr[page_index];
          this.setState(
            {
  ...
  setFilterData = (page_index) => {
    var shadowActiveMenuArr = this.state.shadowActiveMenuArr;
    shadowActiveMenuArr[page_index] = JSON.parse(
      JSON.stringify(this.state.activeMenuArr[page_index])
    );
    this.setState(
      {
        shadowActiveMenuArr,
  ...
  selectFilterLabel = (page_index, box_index, label_index) => {
    let find_index =
      this.state.activeMenuArr[page_index][box_index].indexOf(label_index);
    let activeMenuArr, subData;
    activeMenuArr = this.state.activeMenuArr;
    subData = this.state.subData;
    if (find_index > -1) {
      activeMenuArr[page_index][box_index].splice(find_index, 1);
      subData[page_index].submenu[box_index].submenu[
        label_index
      ].selected = false;
      this.setState(
        {
  ...
  selectRadioLabel = (page_index, box_index, label_index) => {
    ...
    if (activeIndex == label_index) {
      ...
      if (
        activeIndex != null &&
        activeIndex <
          this.state.subData[page_index].submenu[box_index].submenu.length
      ) {
        subData[page_index].submenu[box_index].submenu[
          activeIndex
        ].selected = false;
        ...
        item.forEach((s, j) => {
          if (s != null) {
            ...
            s.forEach((v, k) => {
              value[i][j][k] =
                v == null ||
                v >= this.state.subData[i].submenu[j].submenu.length
                  ? null
                  : this.state.subData[i].submenu[j].submenu[v].value;
              if (
                this.state.subData[i].type == "radio" &&
                value[i][j][k] == null
              ) {
                value[i][j] = [];
                index[i][j] = [];
  ...
  processPage = (index) => {
    ...
    activeMenuArr.splice(
      index,
      1,
      JSON.parse(JSON.stringify(this.state.shadowActiveMenuArr[index]))
    );
    this.setState(
      {
        activeMenuArr,
        ...
        <View className="HMfilterDropdown" catchtouchmove={true}>
          <View className="nav">
            {menu.map((item, index) => {
              return (
                <Fragment key={index}>
                  <View
                    className={"first-menu" + (showPage == index ? " on" : "")}
                    onClick={this.togglePage.bind(this, index)}
                  >
                    <Text className="name">{item.name}</Text>
                    <Text
                      className="iconfont triangle"
          ...
          <View
            className={
              "mask" +
              (isShowMask ? " show" : "") +
              (maskVisibility != true ? " hide" : "")
            }
            onClick={this.togglePage.bind(this, showPage)}
          ></View>
          {subData.map((page, page_index) => {
            return (
              <Fragment key={page_index}>
                <View
                  className={
                    "sub-menu-class" +
                    (showPage == page_index ? " show" : "") +
                    (pageState[page_index] != true ? " hide" : "")
                  }
                >
                  {page.type == "hierarchy" &&
                  ...
                  showPage === page_index ? (
                    <Fragment>
                      <ScrollView
                        className={
                          "sub-menu-list" +
                          (activeMenuArr[page_index].length > 1
                            ? " first"
                            : " alone")
                        }
                        scrollY={true}
                          ...
                          return (
                            <Fragment key={index}>
                              <View
                                ...
                                id={"first_id" + index}
                                onClick={this.selectHierarchyMenu.bind(
                                  this,
                                  page_index,
                                  index,
                                  null,
                            ...
                            <Fragment key={index}>
                              ...
                              sub.submenu.length > 0 ? (
                                <ScrollView
                                  ...
                                  scrollY={true}
                                  scrollIntoView={
                                    "second_id" + secondScrollInto
                                  }
                                >
                                  {sub.submenu.map(
                                    (sub_second, second_index) => {
                                      return (
                                        <Fragment key={second_index}>
                                          <View
                                            className={
                                              "sub-menu" +
                                              (activeMenuArr[page_index][1] ==
                                              second_index
                                                ? " on"
                                                : "")
                                            }
                                          ...
                                          >
                                            ...
                                            sub_second.submenu.length > 0 ? (
                                              <View className="more-sub-menu">
                                                {sub_second.submenu.map(
                                                  (sub2, sub2_index) => {
                                                    return (
                                                      <Fragment
                                                        key={sub2_index}
                                                      >
                                                        {sub_second.showAllSub ||
                                                        sub2_index < 8 ? (
                                                          <Text
                                                            className={
                                                              activeMenuArr[
                                                        ...
                                                        sub_second.submenu
                                                          .length > 9 ? (
                                                          <Text
                                                            onClick={this.showMoreSub.bind(
                                                              this,
                                                              second_index
                                                            )}
                                                          >
                                                            更多
                                                            <Text className="iconfont triangle"></Text>
                                                          </Text>
                                                        ) : null}
                                                      </Fragment>
                                                    );
                      ...
                      <View className="filter">
                        <ScrollView className="menu-box" scrollY={true}>
                          {page.submenu.map((box, box_index) => {
                            return (
                              <View className="box" key={box_index}>
                                ...
                                <View className="labels">
                                  {box.submenu.map((label, label_index) => {
                                    return (
                                      <View
                                        key={label_index}
                                        onClick={this.selectFilterLabel.bind(
                                          this,
                                          page_index,
                                          box_index,
                                          label_index
                        ...
                        <View className="btn-box">
                          <View
                            className="reset"
                            onClick={this.resetFilterData.bind(
                              this,
                              page_index
                            )}
                          >
                          ...
                          <View
                            className="submit"
                            onClick={this.setFilterData.bind(this, page_index)}
                          >
                            确定
                          </View>
                        </View>
                                ...
                                <View className="labels">
                                  {box.submenu.map((label, label_index) => {
                                    return (
                                      <View
                                        key={label_index}
                                        onClick={this.selectRadioLabel.bind(
                                          this,
                                          page_index,
                                          box_index,
                                          label_index
